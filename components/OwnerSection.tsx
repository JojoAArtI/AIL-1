  ...
  return (
    <div id="owner" className="py-16 md:py-32 px-6 md:px-12 bg-[#f4f1e6]">
      <div className="max-w-7xl mx-auto text-center mb-16 md:mb-32">
        <p className="text-[#2d3282] font-bold text-[10px] uppercase tracking-[0.3em] mb-6">Why Work With Us</p>
        <h3 className="font-serif text-3xl sm:text-5xl md:text-7xl text-[#f39223] mb-10 md:mb-16 leading-tight">A Partner You <br className="md:hidden" />Can Trust</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-10">
          {values.map((v, i) => (
            <div key={i} className="bracket-card text-left group hover:-translate-y-2 duration-300">
              <h4 className="font-serif text-2xl mb-4 text-[#2d3282] tracking-tight">{v.title}</h4>
      ...
      <div className="max-w-6xl mx-auto flex flex-col lg:flex-row items-center gap-10 lg:gap-24 border-t border-gray-200 pt-12 md:pt-24">
        <div className="w-full lg:w-1/2 flex justify-center">
          <div className="relative">
            <div className="absolute inset-0 bg-[#2d3282] rounded-full translate-x-4 translate-y-4 -z-10 opacity-20"></div>
            <img
              src="/owner.jpg"
              alt="Rajan Bansal"
              className="rounded-full w-48 h-48 sm:w-64 sm:h-64 md:w-96 md:h-96 object-cover shadow-2xl transition-all duration-700"
            />
            <div className="absolute -bottom-4 -right-4 bg-[#f39223] w-20 h-20 md:w-28 md:h-28 rounded-full flex items-center justify-center text-white font-serif italic text-2xl md:text-4xl shadow-xl">
              R.B
            </div>
          </div>
        </div>
        <div className="w-full lg:w-1/2 space-y-8 text-center lg:text-left">
          <p className="text-[#2d3282] font-bold text-[10px] uppercase tracking-[0.3em]">From the Owner</p>
          <h2 className="text-[#f39223] font-serif text-3xl sm:text-5xl md:text-6xl leading-[1.1]">Our Promise <br className="hidden lg:block" />To You</h2>
          ...
          <div className="pt-6">
            <p className="font-bold text-[#345e37] uppercase tracking-[0.4em] text-[10px] md:text-xs mb-1">Rajan Bansal</p>
            <p className="text-[#2d3282] text-[9px] uppercase tracking-[0.2em] font-bold">Founder & Owner • AIL</p>
          </div>
        </div>
      </div>
    </div>
