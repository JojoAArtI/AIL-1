  ...
  return (
    <div className="py-16 md:py-24 px-6 bg-white">
      <div className="max-w-7xl mx-auto text-center">
        ...
        <h3 className="font-serif text-3xl sm:text-5xl md:text-7xl text-[#f39223] mb-8 md:mb-20 leading-tight">
          ...
          {SERVICES.map((service, idx) => (
            <Link key={service.id} to={service.path} className="group cursor-pointer">
              <div className="relative aspect-[4/5] overflow-hidden shadow-xl mb-8 border border-gray-100">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#f39223]/90 via-transparent to-transparent opacity-40 group-hover:opacity-60 transition-opacity"></div>
              </div>
              <div className="text-left">
                <div className="flex items-center gap-4 mb-4">
                  <span className="text-[#345e37] font-serif text-3xl italic">0{idx + 1}</span>
                  <h4 className="text-[#f39223] font-serif text-2xl md:text-3xl">{service.title}</h4>
                </div>
                <p className="text-gray-500 text-sm leading-relaxed">
                  {service.description}
                </p>
                <button className="mt-6 flex items-center gap-2 text-[#345e37] font-bold text-[9px] uppercase tracking-widest group-hover:gap-3 transition-all">
                  Service Details
          ...
          {SERVICES.map((service, idx) => (
            <Link key={service.id} to={service.path} className="flex gap-4 bg-[#f9f8f4] border border-gray-100 rounded-lg overflow-hidden text-left group cursor-pointer active:scale-[0.98] transition-transform">
              <div className="relative w-24 min-h-[100px] flex-shrink-0 overflow-hidden">
                ...
                />
                <div className="absolute inset-0 bg-[#f39223]/20"></div>
              ...
              <div className="py-3 pr-4 flex flex-col justify-center min-w-0">
                <h4 className="text-[#f39223] font-serif text-lg leading-tight mb-1">{service.title}</h4>
