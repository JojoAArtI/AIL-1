      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 md:mb-20 gap-6 md:gap-8">
          <div className="max-w-xl">
            <p className="text-[#2d3282] font-bold text-[10px] uppercase tracking-[0.3em] mb-4">Our Network</p>
            <h3 className="font-serif text-3xl sm:text-5xl md:text-7xl text-[#f39223] leading-none mb-4 md:mb-6">Pan-India Reach</h3>
          ...
          <button className="text-[#2d3282] font-bold text-[10px] uppercase tracking-widest border-b-2 border-[#2d3282] pb-1 hover:text-[#f39223] hover:border-[#f39223] transition-all">
        ...
        <div className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {BRANCHES.map((branch) => (
            <div key={branch.id} className="group p-10 border border-gray-100 hover:border-[#2d3282] transition-all duration-500 hover:shadow-xl relative overflow-hidden bg-[#fcfcfc]">
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-10">
                  <div className="text-[#345e37]">
                    <ICONS.MapPin />
                  </div>
                ...
                <h4 className="text-[#f39223] font-serif text-3xl mb-4">{branch.location}</h4>
                ...
                <div className="pt-6 border-t border-gray-100 flex flex-col gap-3">
                  <a href={`tel:${branch.phone}`} className="text-[#2d3282] text-[11px] font-bold hover:text-[#f39223] transition-all flex items-center gap-2">
                    <ICONS.Phone />
                    {branch.phone}
                  </a>
        ...
        <div className="md:hidden space-y-3">
          {BRANCHES.map((branch) => (
            <div key={branch.id} className="flex items-center gap-4 p-4 border border-gray-100 bg-[#fcfcfc] rounded-lg active:scale-[0.98] transition-transform">
              <div className="w-10 h-10 rounded-full bg-[#2d3282] flex items-center justify-center text-white flex-shrink-0">
                <ICONS.MapPin />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <h4 className="text-[#f39223] font-serif text-lg">{branch.location}</h4>
              ...
              <a href={`tel:${branch.phone}`} className="w-9 h-9 rounded-full bg-[#f4f1e6] flex items-center justify-center text-[#2d3282] flex-shrink-0 active:bg-[#f39223] active:text-white transition-colors">
                <ICONS.Phone />
              </a>
            </div>
          ))}
